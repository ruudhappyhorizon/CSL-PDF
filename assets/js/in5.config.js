...
function isPopupOverlay($elem) {
	if(!$elem || !$elem.length) { return false; }
	var name = ($elem.attr('name') || '').toLowerCase();
	return name && name.indexOf('popup') !== -1 && name.indexOf('open') === -1 && name.indexOf('close') === -1;
}
...
function hide(dataID) {
	...
	var wasVisible = !$target.hasClass('hidden');
	$target.addClass('hidden');
	if(wasVisible && isPopupOverlay($target)) {
...
function show(dataID) {
	...
	if(wasHidden && isPopupOverlay($target)) {
		openPopupCount += 1;
		updatePopupState();
	}
}
...
function initPageMode(){
	if(multifile){
		...
		var scrollDuration = (pageMode[2] === 'h') ? 0 : 500;
		nav = { numPages:$('.pages .page').length,
			...
			to:function (n,c,q) {
				...
				$.scrollTo(scrollTarg,scrollDuration,{axis:dir, onAfter:function(){ window.scrolling = !1; } });
				if(!scrollDuration){ window.scrolling = !1; }
				$(document).trigger('newPage', { index:n-1 });
				c && nav.highlight(n,c,q);
			}
		};
